"""Camera Input node — capture video frames for vision-based policies.

Provides a live camera feed as tensors for use with vision-based
imitation learning policies. Supports USB webcams via OpenCV.
"""
...
class CameraInputNode(BaseNode):
    """Capture frames from a camera for vision-based robot control."""
    type_name   = "lr_camera"
    label       = "Camera Input"
    category    = "LeRobot"
    description = (
        "Capture frames from a USB camera. Outputs an image tensor "
        "(C, H, W) float in [0, 1] for feeding into vision-based policies. "
    ...
    def export(self, iv, ov):
        return ["import cv2", "import torch", "import numpy as np"], [
            f"# Camera capture — requires OpenCV",
